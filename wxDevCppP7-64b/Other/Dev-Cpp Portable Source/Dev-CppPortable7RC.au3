;added avoid multiple instance
;adapted for wxDev-Cpp from v7RC6 to higher version

#include "AutoItTemplate.au3"

#NoTrayIcon

$DefaultExeName = "wxdevcpp.exe";
...
if ProcessExists($exeName) Then 
  MsgBox(16, "Dev-C++ Portable Launcher Error", "The App is already running.");
  Exit
Endif

if ProcessExists($exeName2) Then 
  MsgBox(16, "Dev-C++ Portable Launcher Error", "The App is already running.");
  Exit
Endif
...
If Not $disableSplashScreen Then
	...
	SplashImageOn("Dev-C++ Portable", @TempDir & "\Dev-CppPortable\Dev-CppPortable.jpg", 217, 95);
...
Func Init()
	If Not FileExists(@ScriptDir & "\Data\wxdevcpp.ini") Then
		Return;
	EndIf
	
	Local $text;
	Local $file = FileOpen(@ScriptDir & "\Data\wxdevcpp.ini", 0);
	...
	$file = FileOpen(@ScriptDir & "\Data\wxdevcpp.ini", 2);
